...
The first version of the mirror gate also had a completeness blind spot: it validated only item headings that were present in an EN package. If a current reconciliation item were omitted from the package entirely, the gate could still pass as long as every remaining visible item had a matching status. That is not acceptable for a controlled 7.1–7.10 review package because omission can hide a stale, unresolved, conflict, or direct-evidence-required item.
...
3. Every current reconciliation ID belonging to a function must be represented in that function's EN review package; omission is a blocking consistency failure.
4. The EN package may keep its own independent `EN status` and `Approval` fields.
5. Updating the FR mirror must **never** promote the EN review state.
6. `BILINGUAL TECHNICAL REVIEW COMPLETE` and `APPROVED` remain prohibited unless the controlled reviewer/sign-off requirements are independently satisfied with a named qualified reviewer and date.
7. If the current FR state is downgraded because direct Tier-A evidence is insufficient, the EN package mirror must follow that downgrade; it must not preserve a more favorable historical label.

## CI correction

`scripts/check-dgr-en-package-fr-status-mirror.mjs` now checks all ten EN review packages against the effective current per-item state. For each function/package it requires:

- every current reconciliation `Q-7.x-###` ID for that function to be present in the package;
...
The regression fixtures now include an omitted-current-item case and require it to fail. The check is wired into `.github/workflows/dgr-regulatory-readiness.yml` and remains intentionally fail-closed. A passing result means only that the EN package is complete against the current reconciliation item set and that duplicated FR status labels are synchronized with the effective per-item state. It does **not** prove regulatory correctness, bilingual equivalence, reviewer qualification, or approval.

## Remediation rule

Regenerate or edit the stale FR mirror fields from the effective per-item state, function by function. Do not omit an item to make a package appear clean, do not bulk-copy Function 7.1 wording or structure into Functions 7.2–7.10, do not turn an explicitly unresolved direct-evidence row into a terminal status merely because its legacy bucket says FROZEN/GAP, and do not use this synchronization work to alter the separately governed EN-review status.
