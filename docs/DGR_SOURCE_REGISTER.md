...
### Q-7.1-001
Need direct current Tier A evidence matching the exact pilot concept for danger vs. risque. **Partial progress 2026-08-24:** §1.0 Définition des marchandises dangereuses (p.11) contains an explicit Note — "La définition des termes ayant un sens spécial dans la présente Réglementation apparaît à l'appendice A" — confirming **Appendice A — Glossaire (Bookshelf p.703)** as the current, correct location for defined terms. The actual "Danger" / "Risque" glossary entries were not yet read (session tooling failure, see below). Do not assume old training-course wording for danger/risque; do not infer the glossary's content.
