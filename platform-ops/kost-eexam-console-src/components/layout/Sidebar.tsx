...
export function Sidebar({
  ...
  return (
    <>
      {/* Fond superposé — mobile uniquement, ferme le tiroir au clic */}
      {mobileOpen && (
        <div
      ...
      <aside
        className={cn(
          ...
          // Desktop (md+) : toujours visible, position normale dans le flux
          // (md:relative — jamais md:static ici : les enfants `absolute` du
          // dégradé décoratif ont besoin d'un ancêtre positionné).
          // Mobile (<768px) : tiroir superposé plein-hauteur (fixed), masqué
          // par défaut hors écran et révélé par le bouton hamburger du
          // Topbar — avant ce correctif, il n'existait aucun moyen
          // d'atteindre la navigation ou la déconnexion en dessous de
          // 768px. Ne JAMAIS combiner une classe `relative` nue avec
          // `fixed` sur le même élément : Tailwind ne garantit pas que
          // `fixed` gagne la cascade sur une utilité `relative` non
          // préfixée (bug réel rencontré ici — la sidebar restait dans le
          // flux normal, réservant 264px, même une fois translatée hors
          // écran) — seule la variante `md:relative` (préfixée, appliquée
          // après par design Tailwind) est sûre à combiner avec `fixed`.
