...
async function login(page, user, pass) {
  ...
  await Promise.race([
    ...
    page.getByText('Identifiants invalides').waitFor({ timeout: 15000 }), // FR copy since 2026-08-25
  ]).catch(() => {});
...
test.describe('Mutation-level RBAC (real write attempts, not just page access)', () => {
  test('auditor is blocked server-side from recording an identity verification (form not even rendered)', async ({ page }) => {
    ...
    // Console UI translated to French 2026-08-25 (this finalization pass).
    await expect(page.locator('body')).toContainText(
      "Votre rôle (Auditeur) n'est pas autorisé à enregistrer des vérifications d'identité"
    );
  ...
  test('FINDING: any authenticated console role (including auditor) can submit /feedback', async ({ page }) => {
    ...
    await page.getByRole('button', { name: 'Donner un retour', exact: true }).click(); // FR since 2026-08-25
    ...
    await page.getByRole('button', { name: '4 étoiles' }).click(); // FR since 2026-08-25
    await page.click('button:has-text("Envoyer le retour")'); // FR since 2026-08-25
    await page.waitForTimeout(1500);
    await expect(page.locator('body')).toContainText('Retour enregistré'); // FR since 2026-08-25
