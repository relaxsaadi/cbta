...
test.describe('Console smoke test (live)', () => {
  ...
  test('invalid credentials are rejected', async ({ page }) => {
    ...
    // was 'Invalid credentials'.
    await expect(page.locator('body')).toContainText('Identifiants invalides');
  });
  ...
  test('admin login succeeds, session persists, logout destroys session', async ({ page }) => {
    ...
    if (viewportWidth >= 768) {
      // Console UI translated to French 2026-08-25 — was 'Log out'.
      const logoutLink = page.getByText('Se déconnecter', { exact: true });
