...
import { listPendingManualGrading } from "@/lib/manual-grading";
...
// Mission "COMPLETE CANDIDATE EXAM LIFECYCLE" (2026-08-29) §55-57 — file
// d'attente de correction manuelle (questions 'short_answer' en mode
// 'manual'). Auditeur : lecture seule structurelle — aucun formulaire de
// notation ne lui est proposé (même convention que /notifications,
// /exam-preparation...).
export default async function GradingPage({ searchParams }: { searchParams: Promise<{ graded?: string; finalized?: string }> }) {
  ...
  const pending = listPendingManualGrading(scopedGroupIdsOrNull(session));
