...
- Monitoring production (santé, DB, disque, CPU/RAM, taux d'erreur, échec sauvegarde, alertes incident) : rien construit — `/system` existe (page) mais pas d'alerting actif. Filets périodiques réels existants à ce jour : balayage chronomètre (§6/§8, `*/5 * * * * sweep.sh`), sauvegarde nocturne + test de restauration hebdomadaire (§10) — tous installés et vérifiés cette session, mais aucun n'alerte activement en cas d'échec (un `docker logs`/consultation `/system` manuelle reste nécessaire).
