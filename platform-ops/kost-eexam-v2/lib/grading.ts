 ...
 * ni correcte ni incorrecte tant qu'un correcteur n'a pas statué. */
function gradeOneQuestion(
  qtype: string,
  correctAnswerSnapshot: string,
  answerJson: string | null
): { pending: boolean; isCorrect: boolean } {
...
export function gradeAttempt(attemptId: number): GradeResult {
  ...
  for (const row of rows) {
    ...
    const { pending, isCorrect } = gradeOneQuestion(row.qtype, row.correct_answer_snapshot, row.answer_json);
    if (pending) {
