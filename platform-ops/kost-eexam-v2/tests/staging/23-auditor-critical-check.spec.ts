...
// l'environnement de revue auditeur, sur des données fraîches créées ici
// (pas les fixtures pilote partagées) pour ne rien perturber.
const RUN_TAG = Date.now();
const FRESH_CANDIDATE_USERNAME = `auditor.check.${RUN_TAG}`;
...
test("vérification auditor-critical complète — 22 points, gel staging 2026-08-27", async ({ page }) => {
  ...
  await page.locator('input[name="fullName"]').fill(`Candidat Vérification Auditeur ${RUN_TAG}`);
  await page.locator('input[name="username"]').fill(FRESH_CANDIDATE_USERNAME);
  await page.locator('input[name="password"]').fill("VerifAuditeur2026!");
  await page.getByRole("button", { name: /ajouter/i }).click();
  await expect(page.getByText(`Candidat Vérification Auditeur ${RUN_TAG}`).first()).toBeVisible();

  // 6. ASSIGN CANDIDATE TO COMPANY/GROUP/FUNCTION + 7. CREATE AND ASSIGN EXAM.
  await page.goto("/exam-preparation");
  await page.locator('input[name="name"]').fill(`Vérification auditeur ${RUN_TAG}`);
  ...
  await page.context().clearCookies();
  await loginAs(page, FRESH_CANDIDATE_USERNAME, "VerifAuditeur2026!");
  ...
  // 11. CANDIDATE RESULT.
  await expect(page.getByText(`Vérification auditeur ${RUN_TAG}`).first()).toBeVisible();
  ...
  const resultHref = await page.getByRole("link", { name: `Candidat Vérification Auditeur ${RUN_TAG}` }).getAttribute("href");
  ...
  await page.locator("#description").fill(`Vérification auditeur — action incident ${RUN_TAG}`);
  ...
  const value = await select.locator("option", { hasText: FRESH_CANDIDATE_USERNAME }).getAttribute("value");
  ...
  await expect(page.getByRole("link", { name: `Candidat Vérification Auditeur ${RUN_TAG}` })).toBeVisible();

  // 20. TENANT ISOLATION.
  await page.context().clearCookies();
  await loginAs(page, env("STAGING_MANAGER_B_USER"), env("STAGING_MANAGER_B_PASS"));
  await page.goto("/results");
  await expect(page.getByText(`Candidat Vérification Auditeur ${RUN_TAG}`)).toHaveCount(0);
  ...
  // 22. FAMILIARIZATION RECORD + ATTENDANCE SHEET PDF.
  ...
  const attendanceRow = page.locator("div.rounded-md.border-border-subtle").filter({ hasText: `Candidat Vérification Auditeur ${RUN_TAG}` }).first();
